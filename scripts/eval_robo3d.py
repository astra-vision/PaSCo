from pasco.data.semantic_kitti.kitti_dm_robo3d import KittiDataModuleRobo3D
...
@click.option('--n_infers', default=2, help="batch size")
...
def main(
    ...
    max_angle, translate_distance,
    n_infers, model_path, iou_threshold,
    dataset_root, dataset_preprocess_root, config_path):
    ...
    # Setup dataloaders
    n_classes = 20
    data_module = KittiDataModuleRobo3D(
        root=dataset_root,
    ...
    model = Net.load_from_checkpoint(model_path, class_frequencies=class_frequencies, iou_threshold=iou_threshold)
    model.cuda()
    model.eval()
