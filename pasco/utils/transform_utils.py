...
def generate_transformation(rot=0.0,
                            translation=(0.0, 0.0, 0.0),  # in meters
                            flip_dim=None,
                            scale=1):
    # * flip
...
def generate_random_transformation(max_angle=45,
                                   flip=True,
                                   scale_range=0.1,
    ...
    return generate_transformation(rot=rot, translation=translation, flip_dim=flip_dim, scale=scale)


def transform(coords, T, resolution=0.2):
    min_bound = torch.tensor(
        [0, -25.6, -2]).reshape(1, 3).to(coords.device)
    max_bound = torch.tensor([51.2, 25.6, 4]).reshape(1, 3).to(coords.device)
    points = coords * resolution + resolution / 2
    # points = vox_origin + points
    points = min_bound + points

    x_bias = (max_bound[:, 0] - min_bound[:, 0]) / 2
    min_bound[:, 0] -= x_bias
    max_bound[:, 0] -= x_bias
    points[:, 0] -= x_bias

    homogenous_points = torch.cat(
        [points, torch.ones(points.shape[0], 1).to(points.device)], dim=1).type_as(T)
...
def transform_scene(from_coords, T, voxel_features):
    """
    ...
    # * 4. Sample the original grid features at the projected grid
    sampled_features = sample_grid_features(
        to_coords_projected, voxel_features)

    return sampled_features.T, to_coords.int()


def sample_scene(min_to_coords: int, T, to_voxel_features, out_scene_size, resolution=0.2):
    """
    Only implement for batch size = 1
    Steps to transform a grid to avoid holes:
    2. Sample the new grid
    ...
    sampled_features = sample_grid_features(
        from_coords_projected, to_voxel_features)
