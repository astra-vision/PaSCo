...
class KittiDatasetRobo3D(Dataset):
    def __init__(
        self,
        split,
        root,
        ...
        assert level in ["heavy", "light", "moderate"]
        ...
        for sequence in self.sequences:
            ...
            for voxel_path in glob.glob(glob_path):
                ...
                if float(frame_id) % 5 != 0:
                    continue
                
                if visualize and frame_id not in frame_ids:
                    continue
    ...
    def load_calib_poses(self):
        ...
        for seq in range(0, 11):
            ...
            # Read times
            self.times.append(
                np.loadtxt(os.path.join(seq_folder, "times.txt"), dtype=float)
            )
