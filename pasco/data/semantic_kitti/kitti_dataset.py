...
import numpy as np
import uncertainty.data.semantic_kitti.io_data as SemanticKittiIO
import yaml
from tqdm import tqdm
import random
import pickle
import torch.nn.functional as F
from uncertainty.data.semantic_kitti.instance_cut_mix import InstanceCutMix
...
from uncertainty.models.transform_utils import transform_scene, transform, generate_random_transformation, transform_xyz, sample_scene, generate_transformation
from uncertainty.data.semantic_kitti.collate import collate_fn
from uncertainty.data.semantic_kitti.params import thing_ids
import random
...
class KittiDataset(Dataset):
    def __init__(
        ...
        self.instance_cut_mix = instance_cut_mix
        print(self.data_aug, self.max_angle, self.scale_range, self.translate_distance)
        print("Instance cut mix", self.instance_cut_mix)
        ...
        for sequence in self.sequences:
            ...
            for voxel_path in glob.glob(glob_path):
                ...
                if float(frame_id) % frame_interval != 0:
                    continue
                if overfit and frame_id != "000155":
                    continue
        ...
        if self.instance_cut_mix:
            self.instance_cut_mix = InstanceCutMix(self.preprocess_root, "train")
            self.instance_cut_mix.load()  

        if self.overfit:
            if self.split == "train": 
                self.scans = self.scans[:100]
            else:
                self.scans = self.scans[:4]
    ...
    def get_individual(self, idx):
        ...
        for scale in scales:
            if scale == 1:
                ...
                downscaled_sem_label_0_255[downscaled_sem_label_0_255_oh[20, :, :, :] == 1] = 0
    ...
    def voxelize(self, xyz, vote):
        ...
        voxel_centers = (coords.astype(np.float32) + 0.5) * self.voxel_size + vox_origin.reshape(1, 3)
    ...
    def load_data_v3(self, sequence, frame_id, downsample=1, n_fuse_scans=1):
        ...
        for fuse_idx in range(n_fuse_scans):
            ...
            path = os.path.join(self.preprocess_root, "waffleiron_v2/sequences", sequence, "seg_feats_tta", "{}.pkl".format(add_frame_id))
            ...
            if os.path.exists(path):
                ...
                if fuse_idx == 0:
                    ...
                    input_pcd_instance_label = self.load_input_pcd_instance_label(sequence, frame_id)
                else:
        ...
        if self.split == "train":
            in_keep, semantic_keep, instance_keep = self.crop(semantic_coords, in_coords, instance_coords)
    ...
    def crop(semantic_coords, in_coords, instance_coords):
        ...
        in_keep = (in_coords[:, 0] >= new_min_coords[0]) & (in_coords[:, 0] < new_max_coords[0]) & \
                  (in_coords[:, 1] >= new_min_coords[1]) & (in_coords[:, 1] < new_max_coords[1])
        semantic_keep = (semantic_coords[:, 0] >= new_min_coords[0]) & (semantic_coords[:, 0] < new_max_coords[0]) & \
                        (semantic_coords[:, 1] >= new_min_coords[1]) & (semantic_coords[:, 1] < new_max_coords[1])
        instance_keep = (instance_coords[:, 0] >= new_min_coords[0]) & (instance_coords[:, 0] < new_max_coords[0]) & \
                        (instance_coords[:, 1] >= new_min_coords[1]) & (instance_coords[:, 1] < new_max_coords[1])
